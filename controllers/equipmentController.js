...
const addUseHours = async (req, res) => {
	...
	try {
		if (equipmentHourBody.hours_to_add < 0)
			return res.status(400).send(CANNOT_ADD_NEGATIVE_HOURS);
		...
		if (!equipment_hour)
			return res.status(400).send(ERROR_CREATING_EQUIPMENT_HOUR);
		...
		if (!equipmentUpdated)
			return res.status(404).send(ERROR_UPDATING_EQUIPMENT);

		if (equipmentUpdated.lubrication_sheet_id)
			await maintenanceController.updateNextMaintenancesForEquipment(
				equipmentUpdated
			);
...
const addLubricationSheetToEquipment = async (req, res) => {
	...
	try {
		...
		if (!equipment) return res.status(404).send(EQUIPMENT_NOT_FOUND);
		const lubrication_sheet =
			await commonController.findLubricationSheetById({
				id: body.lubrication_sheet_id,
			});
		if (!lubrication_sheet)
			return res
				.status(404)
				.send(lubricationSheetController.LUBRICATION_SHEET_NOT_FOUND);
		...
		catchError(
			res,
			error,
			"Error trying to add lubrication sheet to equipment."
		);
	}
};
